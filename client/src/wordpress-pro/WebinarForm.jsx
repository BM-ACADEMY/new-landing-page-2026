...
export default function WebinarForm({ isModal = false, onClose }) {
  ...
  const handleRegister = async () => {
    ...
    // If you deploy a Google Apps Script Web App for WordPress Pro Webinar, add the URL here:
    const WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbzqu4zbORfjrNjzub8AhMyP7RAnSLxvLbCD4yAW9OMr8eOnjanUA39RJCr0d5dv-cgouw/exec";
    ...
    if (WEBHOOK_URL) {
      ...
      const program = "WordPress Pro Webinar";
      let msg = "Hi BM Academy 👋\n\nI'd like to register for the Free Live WordPress Webinar.";
      msg += `\n\n📅 Date: August 15, 2026`;
      msg += `\n📚 Program: ${program}`;
      msg += `\n👤 Name: ${name.trim()}`;
      msg += `\n📞 Mobile: ${phone.trim()}`;
      ...
      msg += "\n\nPlease reserve my seat. Thank you 🙏";
          ...
          {!isModal && (
            <>
              ...
              <p className="text-muted-brand text-[0.94rem] mt-3 mb-[22px]">
                Learn the exact roadmap to build stunning websites with WordPress. Fill out the form below to secure your spot.
              </p>
            </>
          )}
  ...
  return (
    <section id="webinar-register" className="py-[58px] bg-bg-dark">
