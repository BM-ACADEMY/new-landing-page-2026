...
export default function Hero({ onBookClick }) {
  ...
  useEffect(() => {
    ...
    if (campRef.current) {
      observer.observe(campRef.current);
    }

    return () => {
      if (campRef.current) {
        observer.unobserve(campRef.current);
      }
    };
  }, []);
      ...
      <div className="max-w-[1080px] mx-auto px-5 grid grid-cols-1 md:grid-cols-[1.04fr_.96fr] gap-[44px] items-center relative">
        <div>
          ...
          <p className="text-[#C8C2DC] text-[1rem] sm:text-[1.1rem] mt-[18px] mb-[28px] max-w-[540px]">
            Fill out the form and our <b className="text-white">Digital Marketing</b> expert will contact you for a <b className='text-white'>FREE</b> one-on-one career guidance session. Get personalized advice based on your <b className='text-white'>Education, Skills, and Career Goals.</b></p>
