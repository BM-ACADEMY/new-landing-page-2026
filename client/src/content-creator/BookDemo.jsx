...
export default function BookDemo({ selectedProgram, isModal = false, onClose }) {
  ...
  const [track, setTrack] = useState('Digital Content Creator');
  ...
  const handleBook = async () => {
    ...
    const WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbw6TlZzOLm1Gor372s-4jkT6iSS80HwPIZ5YRLvskBMTjeHvsVXHHHnxzOz6bkcwV2L/exec";
    if (WEBHOOK_URL) {
      fetch(WEBHOOK_URL, {
        method: "POST",
        mode: "no-cors",
    ...
    <div className={`bg-panel border border-white/9 rounded-2xl p-[30px_26px] mx-auto text-left relative ${isModal ? 'w-full' : 'max-w-[560px]'}`}>
      ...
      {isSuccess ? (
        <div className="text-center py-8">
          ...
          <h3 className="font-sora font-extrabold text-[1.4rem] text-white mb-2">Demo Booked Successfully!</h3>
          <p className="text-muted-brand text-[0.9rem] leading-relaxed max-w-[360px] mx-auto">
            Thank you, <strong className="text-white">{name}</strong>! We will contact you at <span className="text-pink-brand font-semibold">{phone}</span> to confirm your slot.
          </p>
        </div>
      ) : (
        <>
          ...
          <p className="text-muted-brand text-[0.94rem] mt-3 mb-[22px]">
            Get full clarity on skills, placement, and EMI options. Submit the form to book your slot.
          </p>
