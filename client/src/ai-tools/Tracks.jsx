    ...
    <section id="tracks" className="py-[58px]">
      <div className="max-w-[1080px] mx-auto px-5 text-center">
        ...
        <div className="mt-4 inline-flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-full text-[0.92rem]">
          ...
          <span className="text-pink-brand font-extrabold text-[1.1rem]">₹8,999</span>
        </div>
        ...
        <div className="max-w-[440px] mx-auto mt-12">
          <div className="reveal bg-gradient-to-b from-[#1b1236] to-[#0c081f] border border-violet-brand/35 shadow-[0_0_30px_rgba(124,92,252,0.15)] rounded-2xl p-[35px_30px] relative text-left transition-all duration-300 hover:scale-[1.01] hover:shadow-[0_0_40px_rgba(124,92,252,0.25)] hover:border-violet-brand/50">
            ...
            <div className="font-extrabold text-pink-brand text-[0.8rem] uppercase tracking-[0.06em] mt-2 mb-4">AI Tools Mastery</div>
            <div className="text-[0.84rem] text-muted-brand mb-6">EMI available · 4 weeks · Hybrid</div>

            <ul className="list-none p-0 m-0 grid gap-[12px] text-[0.92rem] text-[#DAD4EC] mb-8">
              <li className="flex gap-2.5 items-center"><span className="text-green-brand font-bold text-lg">✓</span> Power prompting (ChatGPT, Gemini, Claude)</li>
              <li className="flex gap-2.5 items-center"><span className="text-green-brand font-bold text-lg">✓</span> AI content &amp; image generation</li>
              <li className="flex gap-2.5 items-center"><span className="text-green-brand font-bold text-lg">✓</span> Automation tools setup</li>
              <li className="flex gap-2.5 items-center"><span className="text-green-brand font-bold text-lg">✓</span> Certificate of Completion</li>
            </ul>

            <button 
              onClick={onBookClick}
              className="w-full inline-flex items-center justify-center gap-2.5 font-bold text-base px-[26px] py-[15px] rounded-xl cursor-pointer border-none bg-gradient-to-r from-pink-brand via-violet-brand to-blue-brand text-white transition-all duration-200 hover:brightness-[1.1] w-full"
            >
              Book Demo
            </button>
          </div>
